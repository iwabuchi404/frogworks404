...
| 操作 | Notion MCP | ContextMixer |
|---|---|---|
| プロジェクトの概要を知る | ページ全文取得（数千トークン） | `meta` 取得（数十トークン） |
| 設計判断を確認 | ページ全文取得 → AIが抽出 | `section` で該当箇所だけ |
| コンテキスト組み立て | 4ページ分を手動で取得 | `bundle` で一発取得（予定） |
