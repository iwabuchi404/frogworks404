...
FubakoをAIと開発していて気づいたのは、Electronの不具合の多くはレンダラーの画面上ではなく、メインプロセスとの間、IPCの向こう側で起きるということだった。

たとえばFubakoの画像アップロードが壊れたときがそうだった。`check-ui`で撮ったスクリーンショットでは、アップロードボタンもフォームも正常に表示されている。DOMを渡してもAIは「問題なさそうです」と返すだけ。しかし実際には、Electronの`contextIsolation: true`環境で`<input type="file">`の`file.path`が空文字列になっていて、ファイルパスを取得できず画像コピーが失敗していた。画面の向こう側、IPCハンドラの向こうで起きていることだった。修正は`dialog.showOpenDialog`をIPC経由で呼ぶ方式への切り替えで、これもまた画面からは見えない変更だった。

似た話がもう一つある。プレビューのビルドエラーがレンダラーに届かない現象があった。Zolaのserveプロセスはstderrにエラーを出しているのに、スクショを撮ってもプレビュー画面は正常に見える。原因はメインプロセス側の`event.sender.send('zola-error', ...)`の1行がコメントアウトされていて、IPCでエラーを送る経路が切れていたことだった。エラーはメインプロセスの中で吸われていた。画面とDOMだけを見ていたら、プレビューが壊れている理由は永遠に分からなかった。

Fubakoのコンテンツ削除でも同じ断絶があった。`deleteContent`という関数はメインプロセスに実装されていたが、IPCハンドラが未登録で、`preload.cjs`にもAPIとして公開されていなかった。結果としてUIに削除ボタンすら出ない。レンダラー側のDOMをいくら調べても「削除機能が存在しない」ことしか分からず、実装はあるのに届いていないという断絶は、IPCのログを見るまで見えなかった。
